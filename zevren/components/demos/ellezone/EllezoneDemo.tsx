    ...
    <div className="relative overflow-hidden rounded-2xl border border-[#E4DFF0] bg-white text-[#241F2E]">
      ...
      <nav className="flex items-center justify-between gap-4 border-b border-[#EFEBF7] bg-white px-6 py-5 sm:px-9">
        ...
        <div className="hidden items-center gap-7 text-sm text-[#6B6577] sm:flex">
          ...
          <button type="button" onClick={() => setView("wishlist")} className="hover:text-[#4C3A78]">
            {dict.navWishlist} ({wishlist.length})
          </button>
          <button type="button" onClick={() => setView("account")} className="hover:text-[#4C3A78]">
            {dict.navAccount}
          </button>
        </div>
        <button
          ...
          className="relative flex items-center gap-2 rounded-full border border-[#E4DFF0] px-4 py-2 text-xs font-semibold uppercase tracking-wider text-[#4C3A78] transition-colors hover:border-[#B9AEDC]"
        >
          {dict.cart}
          {cartCount > 0 && (
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#5B4A93] text-[10px] font-semibold text-white">
              {cartCount}
            </span>
          )}
        </button>
      ...
      {view === "home" && (
        <>
          <section className="relative overflow-hidden border-b border-[#EFEBF7] bg-gradient-to-br from-[#F7F5FC] via-white to-[#FDF6F3] px-5 py-14 sm:px-8 sm:py-20">
            <div className="grid items-center gap-10 sm:grid-cols-2">
              <div>
                <span className="text-xs font-semibold uppercase tracking-[0.3em] text-[#5B4A93]">
                  {dict.heroTagline}
                </span>
                <h2 className="mt-4 max-w-md font-heading text-3xl font-semibold leading-tight text-[#241F2E] sm:text-4xl">
                  {dict.heroHeading}
                </h2>
                <p className="mt-4 max-w-sm text-sm leading-relaxed text-[#565064]">
                  {dict.heroSubtitle}
                </p>
                <button
                  type="button"
                  onClick={() => setView("shop")}
                  className="mt-6 rounded-full bg-[#5B4A93] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#F7F5FC]0"
                >
                  {dict.shopButton}
                </button>
              </div>
          ...
          <section className="border-b border-[#EFEBF7] px-5 py-12 sm:px-8">
            <div className="grid gap-6 sm:grid-cols-3">
              {ELLEZONE_FEATURES.map((f) => (
                <div key={f.title} className="rounded-xl border border-[#EFEBF7] bg-[#F7F5FC]/40 p-5">
                  <p className="text-sm font-semibold text-[#241F2E]">{f.title}</p>
                  <p className="mt-1.5 text-sm leading-relaxed text-[#565064]">{f.body}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="px-5 py-12 sm:px-8">
            <h3 className="text-xs font-semibold uppercase tracking-[0.3em] text-[#5B4A93]">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-3">
              {ELLEZONE_PRODUCTS.map((p) => (
                <button
                  ...
                  className="group flex flex-col overflow-hidden rounded-xl border border-[#EFEBF7] bg-white text-left transition-shadow hover:shadow-lg"
                >
                  ...
                  <div className="flex flex-1 flex-col gap-1 p-4">
                    <span className="text-sm font-medium text-[#241F2E]">{p.name}</span>
                    <span className="text-xs text-[#6B6577]">{p.tagline}</span>
                    <span className="mt-1 text-sm font-semibold text-[#4C3A78]">
      ...
      {view === "shop" && (
        <section className="px-5 py-12 sm:px-8">
          <h2 className="font-heading text-2xl font-semibold text-[#241F2E]">
            Kitchen Storage
          </h2>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
            <input
              ...
              className="flex-1 rounded-xl border border-[#E4DFF0] px-3 py-2 text-sm text-[#241F2E] outline-none focus:border-violet-500"
            />
            <div className="flex gap-2">
              <button
                ...
                className={`rounded-full border px-4 py-1.5 text-xs font-medium uppercase tracking-wider transition-colors ${
                  colorFilter === "all"
                    ? "border-violet-600 bg-[#5B4A93] text-white"
                    : "border-[#E4DFF0] text-[#565064] hover:border-[#B9AEDC]"
                }`}
              >
                {dict.allColours}
              </button>
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-3">
            {filteredProducts.map((p) => (
              <div
                key={p.id}
                className="group relative flex flex-col overflow-hidden rounded-xl border border-[#EFEBF7] bg-white transition-shadow hover:shadow-lg"
              >
                <button
                  type="button"
                ...
                <div className="flex flex-1 flex-col gap-1 p-4">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <span className="text-sm font-semibold text-[#4C3A78]">
                      &euro;{p.price.toFixed(2)}
                    </span>
                    {p.compareAtPrice && (
                  ...
                  <button
                    type="button"
                    onClick={() => openProduct(p)}
                    className="mt-2 w-fit text-xs font-semibold text-[#4C3A78] hover:text-[#5B4A93]"
                  >
                    View product
                  </button>
                </div>
            ...
            {filteredProducts.length === 0 && (
              <p className="col-span-full text-sm text-[#6B6577]">
      ...
      {view === "product" && activeProduct && (
        <section className="px-5 py-12 sm:px-8">
          <button
            type="button"
            onClick={backToShop}
            className="mb-6 text-xs font-medium text-[#6B6577] hover:text-[#4C3A78]"
          >
            &larr; {common.back}
          </button>
          <div className="grid gap-8 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xl font-semibold text-[#4C3A78]">
              ...
              <p className="text-sm leading-relaxed text-[#565064]">
                {activeProduct.description}
              </p>
              <div>
                <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#6B6577]">
                  {dict.colour}
                </p>
              ...
              <div className="mt-2 flex flex-wrap gap-3">
                <button
                  type="button"
                  onClick={addToCart}
                  className="rounded-full bg-[#5B4A93] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#F7F5FC]0"
                >
                  {dict.addToCart}
                </button>
                <button
      ...
      {view === "wishlist" && (
        <section className="px-5 py-12 sm:px-8">
          <h2 className="font-heading text-2xl font-semibold text-[#241F2E]">{dict.wishlistHeading}</h2>
          {wishlist.length === 0 ? (
            <p className="mt-4 text-sm text-[#6B6577]">{dict.wishlistEmpty}</p>
          ) : (
            <div className="mt-6 grid gap-5 sm:grid-cols-3">
              {ELLEZONE_PRODUCTS.filter((p) => wishlist.includes(p.id)).map((p) => (
                <button
                  ...
                  type="button"
                  onClick={() => openProduct(p)}
                  className="flex flex-col overflow-hidden rounded-xl border border-[#EFEBF7] bg-white text-left transition-shadow hover:shadow-lg"
                >
      ...
      {view === "account" && (
        <section className="px-5 py-12 sm:px-8">
          <h2 className="font-heading text-2xl font-semibold text-[#241F2E]">{dict.accountHeading}</h2>
          <p className="mt-2 text-xs text-stone-400">{dict.accountNote}</p>
          <div className="mt-8 grid gap-8 sm:grid-cols-2">
            <div>
              <h3 className="text-xs font-semibold uppercase tracking-wider text-[#6B6577]">
                {dict.profileHeading}
              </h3>
              <div className="mt-3 flex flex-col gap-2 text-sm text-[#565064]">
            ...
            <div>
              ...
              {orderHistory.length === 0 ? (
                <p className="mt-3 text-sm text-[#6B6577]">{dict.noOrdersYet}</p>
              ) : (
                <div className="mt-3 flex flex-col gap-3">
                  {orderHistory.map((order) => (
                    <div key={order.number} className="rounded-2xl border border-[#EFEBF7] p-3 text-sm">
                      <p className="font-semibold text-[#241F2E]">{order.number}</p>
                      <p className="text-[#6B6577]">{order.items}</p>
                      <p className="text-[#6B6577]">&euro;{order.total.toFixed(2)}</p>
                    </div>
                  ))}
                </div>
              )}
      ...
      {view === "checkout" && (
        <section className="px-5 py-12 sm:px-8">
          ...
          <h2 className="font-heading text-2xl font-semibold text-[#241F2E]">{dict.checkoutHeading}</h2>
          ...
          <div className="mt-6 grid gap-10 sm:grid-cols-[1.2fr_1fr]">
            <form onSubmit={handleCheckoutSubmit} className="flex flex-col gap-4">
              <div>
                <label className="mb-1.5 block text-xs font-medium text-[#6B6577]">
                  {common.fullName}
                </label>
                <input
                  value={customer.name}
                  onChange={(e) => setCustomer({ ...customer, name: e.target.value })}
                  className="w-full rounded-xl border border-[#E4DFF0] bg-white px-3 py-2.5 text-sm text-[#241F2E] outline-none focus:border-violet-500"
              ...
              <button
                type="submit"
                className="mt-2 w-fit rounded-full bg-[#5B4A93] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#F7F5FC]0"
              >
                {dict.placeOrderDemo}
              </button>
            </form>
            <div className="flex flex-col gap-3 rounded-xl border border-[#EFEBF7] bg-[#F7F5FC]/30 p-5 h-fit">
              <p className="text-xs font-semibold uppercase tracking-wider text-[#6B6577]">
                {dict.orderSummary}
              </p>
              {cart.map((l) => (
                <div
              ...
              <div className="mt-2 flex items-center justify-between border-t border-[#EFEBF7] pt-3 text-sm text-stone-700">
                <span>Subtotal</span>
              ...
              <div className="flex items-center justify-between border-t border-[#EFEBF7] pt-3 text-sm font-semibold text-[#241F2E]">
      ...
      {view === "confirmed" && (
        <section className="flex flex-col items-center gap-4 px-5 py-20 text-center sm:px-8">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-violet-100 text-[#4C3A78]">
            <svg viewBox="0 0 20 20" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth={2}>
              <path d="m4 10 4 4 8-8" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
          <p className="text-xl font-semibold text-[#241F2E]">{dict.orderConfirmedTitle}</p>
          <p className="max-w-sm text-sm leading-relaxed text-[#565064]">
            {dict.orderConfirmedBody}
          </p>
          {orderNumber && (
            <p className="text-sm text-[#6B6577]">
              {dict.orderNumber} <span className="font-semibold text-[#241F2E]">{orderNumber}</span>
            </p>
          )}
          <button
            type="button"
            onClick={backToShop}
            className="mt-2 rounded-full border border-[#E4DFF0] px-5 py-2 text-xs font-semibold uppercase tracking-wider text-stone-700 transition-colors hover:border-[#B9AEDC]"
          >
            {common.back}
          </button>
        </section>
        ...
        <div className="absolute inset-0 z-10 flex justify-end bg-stone-900/40">
          <div className="flex h-full w-full max-w-sm flex-col bg-white p-5 sm:p-6">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold uppercase tracking-wider text-[#241F2E]">
                {dict.cart}
              </p>
              <button
                type="button"
                onClick={() => setCartOpen(false)}
                className="text-stone-400 hover:text-[#241F2E]"
            ...
            {cart.length === 0 ? (
              <p className="mt-8 text-sm text-[#6B6577]">Your cart is empty.</p>
            ) : (
                  ...
                  return (
                    <div key={key} className="flex gap-3">
                      ...
                      <div className="flex flex-1 flex-col gap-1">
                        <span className="text-sm font-medium text-[#241F2E]">{l.product.name}</span>
                        <span className="text-xs text-[#6B6577]">{variant?.label}</span>
                        <div className="mt-1 flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() => changeQty(key, -1)}
                            className="flex h-6 w-6 items-center justify-center rounded-full border border-[#E4DFF0] text-xs text-[#565064] hover:border-[#B9AEDC]"
                          >
                            -
                          </button>
                      ...
                      <span className="text-sm font-semibold text-[#241F2E]">
            ...
            <div className="mt-6 border-t border-[#EFEBF7] pt-4">
              <div className="flex items-center justify-between text-sm font-semibold text-[#241F2E]">
                <span>Subtotal</span>
                <span>&euro;{subtotal.toFixed(2)}</span>
              </div>
              <button
                ...
                className="mt-4 w-full rounded-full bg-[#5B4A93] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#F7F5FC]0 disabled:cursor-not-allowed disabled:opacity-40"
              >
