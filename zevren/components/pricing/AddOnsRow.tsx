        ...
        {ADD_ONS.map((addOn) => {
          ...
          return (
            <div
              key={addOn.key}
              data-reveal-item
              className="flex items-start justify-between gap-5 rounded-2xl border border-white/10 bg-navy/50 p-5"
            >
              <div className="flex flex-col gap-1.5">
                <dt className="text-sm font-semibold text-white">{copy.name}</dt>
                <dd className="text-sm leading-relaxed text-muted">{copy.description}</dd>
              </div>
              <div className="flex shrink-0 flex-col items-end">
                <span className="font-heading text-xl font-bold leading-none text-white">
                  &euro;{addOn.price}
                </span>
                <span className="mt-1 text-[11px] uppercase tracking-[0.12em] text-muted">
                  {copy.unit}
                </span>
              </div>
            </div>
